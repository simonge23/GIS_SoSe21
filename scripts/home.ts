generateRezepte("https://gissose2020simon.herokuapp.com/rezepte", async (rezept: Rezept) => {
    let result;
    ...
    if (clickedButton.dataset.liked === "true") {
        ...
        result = await fetch(`https://gissose2020simon.herokuapp.com/rezepte/likedby/add?user=${localStorage.getItem("email")}&id=${rezept._id}`);
        let resultJSON = await result.json();
        if (resultJSON.successful_updated === true) {
